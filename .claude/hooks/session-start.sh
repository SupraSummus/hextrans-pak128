...
# `landscape/grounds/texture_lightmap/`, plus bespoke supervised
# models like `infrastructure/rail_bridges/rail_060_bridge/`). Tools
# shared by both pipelines live in `tools/threed/`. Idempotent.
set -euo pipefail
...
# Python deps used by tools/threed/{render,diff,crop_ref}.py and the
