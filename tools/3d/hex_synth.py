...
def build_atlas(geom: HexGeom, cols: int, render_cell):
    """Bake a packed atlas of one cell per valid slope.

    Cells are written in `iter_valid_slopes()` order into a (cols × rows)
    grid; sparsity lives in the .dat (raw `slope_t` index → atlas cell),
    not in the PNG.  `render_cell(slope, geom)` returns an HxWx4 RGBA
    array sized `(geom.h, geom.w)`.

    Returns `(atlas_rgba, layout, rows)` where `layout` is the
    `[(slope, row, col), …]` list in pakset emission order.
    """
